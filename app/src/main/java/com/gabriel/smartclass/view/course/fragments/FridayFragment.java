...
public class FridayFragment extends Fragment {
    ...
    public void initialize() {
        ...
        this.binding.addFridayButton.setOnClickListener(this.addSubjectListener());
        if(!this.viewModel.isShowEdition()){
            binding.fridaysubjects.setVisibility(View.GONE);
            binding.addFridayButton.setVisibility(View.GONE);
        }
    }
    ...
    private void addSubject() {
        String subject = Objects.requireNonNull(this.binding.fridaysubjects.getText()).toString();
        if(!subject.equals("")){
    ...
    private void subjectsObserver() {
        this.viewModel.getFridaySubjectsLiveData().observe(this.getViewLifecycleOwner(), observerList());
    }
    ...
    private void buildRecyclerView() {
        this.binding.rcFriday.setHasFixedSize(true);
        this.binding.rcFriday.setLayoutManager(new LinearLayoutManager(this.getContext()));
        this.binding.rcFriday.setAdapter(adapter);
        this.adapter.setRemoveSubject(index -> this.viewModel.removeSubject(index, DAY));
    }

    private void emptyView() {
        EmptyTimetableBinding emptyBinding = binding.emptyFriday;
        EmptyRecyclerViewObserver emptyRecyclerViewObserver = new EmptyRecyclerViewObserver(binding.rcFriday, emptyBinding.getRoot());
